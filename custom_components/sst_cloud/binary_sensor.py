...
async def async_setup_entry(hass, config_entry, async_add_entities):
    ...
    for module in sst1.devices:
        ...
        if module.get_device_type == 1:
            new_devices.append(Thermostat(module))
    if new_devices:
...
class WirelessLeakSensorAlert(Entity):
    ...
    def __init__(self, wirelessLeakSensor, module):
        ...
        self._attr_unique_id = (
            f"{self._sensor.get_wireless_leak_serial_number}_WireLessLeakSensorAlert"
        )
        # Отображаемое имя
...
class LeakSensorAlert(Entity):
    ...
    def __init__(self, leakSensor: sst.LeakSensor, module: sst.LeakModule):
        ...
        # Уникальный идентификатор
        self._attr_unique_id = (
            f"{self._sensor.get_leak_sensor_name}_leakSensorAlertOfModule"
            + str(self._module.get_device_id)
        )
        # Отображаемое имя
...
class WirelessLeakSensorLost(Entity):
    ...
    def __init__(self, wirelessLeakSensor, module: sst.LeakModule):
        ...
        # Уникальный идентификатор
        self._attr_unique_id = (
            f"{self._sensor.get_wireless_leak_serial_number}_WirelesleakSensorLost"
        )
        # Отображаемое имя
        self._attr_name = (
            f"WirelessLeakSensor Lost {self._sensor.get_wireless_leak_sensor_name}"
        )
        # Текущее значение
...
class WirelessLeakSensorBatteryDischarge(Entity):
    ...
    def __init__(
        self, wirelessLeakSensor: sst.WirelessLeakSensor, module: sst.LeakModule
    ):
        self._sensor = wirelessLeakSensor
        self._module = module
        # Уникальный идентификатор
...
class MainModule(Entity):
    ...
    def __init__(self, module: sst.LeakModule):
        ...
        self._module = module
        # self.is_on = False
        if (self._module.second_group_alarm == "true") | (
            self._module.first_group_alarm == "true"
        ):
            self._is_on = True
        else:
            self._is_on = False
...
class MainModuleNeptunProw(Entity):
...
class Thermostat(Entity):
    ...
    def __init__(self, module: sst.ThermostatMCS350):
        ...
        self._module = module
        self._is_on = self._module._relay_status
        # Уникальный идентификатор
        self._attr_unique_id = f"{self._module.get_device_id}_ThermostatSST"
        # Отображаемое имя
        self._attr_name = f" Thermostat SST {self._module.get_device_name}"
